...
def list_tracked_tokens(update: Update, context: CallbackContext):
    ...
    ).get_tracked_tokens(chat_data['group_id'])
    ...
    if len(tracked_tokens) == 0:
        ...
        for token in tracked_tokens:
            message += f"<b>{token.token_symbol}</b>\n"
            message += f"<b>Token Address: </b><i>{token.token_address}</i>\n"
            message += f"<b>Pair Address: <i>{token.pair_address}</i>\n\n"
